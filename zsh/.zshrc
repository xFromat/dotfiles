...
plugins=(git virtualenv virtualenvwrapper python nvm npm)
...
# Start the SSH agent and load all keys
if [ -z "$SSH_AUTH_SOCK" ]; then
  eval "$(ssh-agent -s)" &> /dev/null
fi

#for KEY in /home/peter/.ssh/*.pub; do
...
IDF_PATH='/home/peter/esp/v5.3/esp-idf'
# <<< esp-idf <<<
...
# Add TeX Live's info pages to INFOPATH
...
tmx () {
    local SESSION_NAME="${1:-SessionName}"  # default to SessionName if no arg
